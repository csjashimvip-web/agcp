...
export default function DashboardPage() {
  ...
  return <main className="portal"><PortalHeader name={user.name} admin={isAdmin}/><section className="shell portalHero"><div><p className="eyebrow">Secure workspace</p><h1>Good to see you, {user.name.split(" ")[0]}.</h1><p>Manage identity, wallet balances, catalog shopping and orders from one place.</p></div><div className="accountBadge"><span>{user.name.slice(0,1).toUpperCase()}</span><div><strong>{user.email}</strong><small>{user.status}</small></div></div></section>{message?<div className="shell notice success">{message}</div>:null}<section className="shell portalGrid five">{cards.map(([title,value,desc])=><article className="infoCard" key={title}><small>{title}</small><h3>{value}</h3><p>{desc}</p></article>)}</section><section className="shell actionPanel"><div><p className="eyebrow">Recommended next step</p><h2>{user.two_factor_enabled?"Review active sessions":"Protect the account with 2FA"}</h2><p>{user.two_factor_enabled?"Remove any device or session that you do not recognize.":"Administrative access remains locked until two-factor authentication is confirmed."}</p></div><div className="actions"><Link className="primary" href="/catalog">Browse catalog</Link><Link className="secondary" href="/orders">View orders</Link><Link className="secondary" href="/wallet">Open wallet</Link><Link className="secondary" href="/security">Security center</Link>{!user.email_verified?<button className="secondary" onClick={resend}>Resend verification</button>:null}{isAdmin&&user.two_factor_enabled?<Link className="secondary" href="/admin">Open administration</Link>:null}</div></section></main>;
}
