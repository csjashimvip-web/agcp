...
final class OrderController extends Controller
{
    ...
    { return OrderResource::collection(Order::query()->with('items')->where(['tenant_id'=>$tenant->requireId(),'user_id'=>$request->user()->id])->latest()->paginate(20)); }
    public function show(Request $request,Order $order,TenantContext $tenant):OrderResource
    { abort_unless($order->tenant_id===$tenant->requireId()&&$order->user_id===$request->user()->id,404); return new OrderResource($order->load(['items','statusHistory'])); }
